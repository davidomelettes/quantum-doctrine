...
function Timepicker() {
	...
	this.tpDiv = $('<div id="'+this._divId+'" class="ui-timepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"/>');
}
$.extend(Timepicker.prototype, {
	...
	_newInstance: function(target, options) {
		if (!target[0].id) {
			this.uuid += 1;
			target[0].id = 'tp' + this.uuid;
		}
		return {id: target[0].id, input: target, tpDiv: this.tpDiv, opts: options};
	},
	...
	_showTimepicker: function(input) {
		...
		instance.tpDiv.css({display:'none', position: 'absolute', top: offset.top+'px', left:offset.left+'px'});
